/* empty css                                        */
import { e as createComponent, f as createAstro, k as renderComponent, r as renderTemplate, m as maybeRenderHead, h as addAttribute, u as unescapeHTML } from '../../chunks/astro/server_BmgwVtGw.mjs';
import 'piccolore';
...
const $$slug = createComponent(($$result, $$props, $$slots) => {
  ...
  const relatedPosts = db.prepare("SELECT * FROM posts WHERE category = ? AND id != ? LIMIT 3").all(post.category, post.id);
  return renderTemplate`${renderComponent($$result, "Layout", $$Layout, { "title": `${post.title} - \u0130zmir Uzman\u0131` }, { "default": ($$result2) => renderTemplate` ${maybeRenderHead()}<div class="h-[40vh] md:h-[60vh] relative"> <img${addAttribute(post.image || `https://picsum.photos/seed/${post.id}/1200/600`, "src")}${addAttribute(post.title, "alt")} class="w-full h-full object-cover"> <div class="absolute inset-0 bg-black/40 flex items-center justify-center text-center p-4"> <div class="text-white max-w-4xl"> <span class="inline-block px-4 py-1 bg-primary text-white rounded-full text-sm font-semibold mb-4">${post.category}</span> <h1 class="text-3xl md:text-5xl font-bold mb-4">${post.title}</h1> <p class="text-lg opacity-90">${new Date(post.created_at).toLocaleDateString("tr-TR", { year: "numeric", month: "long", day: "numeric" })}</p> </div> </div> </div> <div class="container mx-auto px-4 py-12 grid grid-cols-1 lg:grid-cols-12 gap-12"> <main class="lg:col-span-8"> <article class="prose prose-lg max-w-none prose-headings:text-dark prose-a:text-primary hover:prose-a:text-primary/80 prose-img:rounded-card"> <p class="lead font-medium text-xl text-gray-700 mb-8">${post.excerpt}</p> <div>${unescapeHTML(post.content.replace(/\n/g, "<br/>"))}</div> </article> <div class="mt-12 pt-8 border-t border-gray-100"> <h3 class="font-bold text-dark mb-4">Paylaş</h3> <div class="flex gap-4"> <button class="w-10 h-10 rounded-full bg-[#1877F2] text-white flex items-center justify-center hover:opacity-90 transition">
F
