...
const $$slug = createComponent(async ($$result, $$props, $$slots) => {
  ...
  return renderTemplate`${renderComponent($$result, "Layout", $$Layout, { "title": `${postData.title} - \u0130zmir Uzman\u0131`, "seoTitle": postData.seo_title, "seoDescription": postData.seo_description, "description": postData.excerpt }, { "default": async ($$result2) => renderTemplate` ${maybeRenderHead()}<div class="h-[40vh] md:h-[60vh] relative"> <img${addAttribute(postData.image || `https://picsum.photos/seed/${postData.id}/1200/600`, "src")}${addAttribute(postData.title, "alt")} class="w-full h-full object-cover"> <div class="absolute inset-0 bg-black/40 flex items-center justify-center text-center p-4"> <div class="text-white max-w-4xl"> <span class="inline-block px-4 py-1 bg-primary text-white rounded-full text-sm font-semibold mb-4">${postData.category}</span> <h1 class="text-3xl md:text-5xl font-bold mb-4">${postData.title}</h1> <p class="text-lg opacity-90">${new Date(postData.created_at).toLocaleDateString("tr-TR", { year: "numeric", month: "long", day: "numeric" })}</p> </div> </div> </div> <div class="container mx-auto px-4 py-12 grid grid-cols-1 lg:grid-cols-12 gap-12"> <main class="lg:col-span-8"> <article class="prose prose-lg max-w-none prose-headings:text-dark prose-a:text-primary hover:prose-a:text-primary/80 prose-img:rounded-card"> <p class="lead font-medium text-xl text-gray-700 mb-8">${postData.excerpt}</p> ${renderComponent($$result2, "Content", Content, {})} </article> <div class="mt-12 pt-8 border-t border-gray-100"> <h3 class="font-bold text-dark mb-4">Paylaş</h3> <div class="flex gap-4"> <button class="w-10 h-10 rounded-full bg-[#1877F2] text-white flex items-center justify-center hover:opacity-90 transition">
F
